...
use clap::Parser;
use datafusion::{
    arrow::{self, array::ArrayRef, util::pretty::pretty_format_batches}, physical_plan::display::DisplayableExecutionPlan, prelude::{SessionConfig, SessionContext}
};
...
async fn exec_plan(
    ...
    write_all_to_single_csv_line(path, durations, res, &outputname)?;

    Ok(())
}
fn write_all_to_single_csv_line(file_path: PathBuf, durations: Vec<Duration>, batches: Vec<RecordBatch>, output_path: &str) -> Result<(), Box<dyn Error>> {
    use std::{fs::File, error::Error, path::PathBuf, time::Duration}; 
    use arrow::{record_batch::RecordBatch, csv::Writer, array::ArrayRef}; 
    use csv::Writer as CsvWriter;

    let file_name = file_path.file_name().and_then(|n| n.to_str()).unwrap_or("unknown"); 
    let mut row: Vec<String> = vec![file_name.to_string(), "timing".to_string()]; 
    row.extend(durations.iter().map(|d| d.as_millis().to_string())); 
    row.push("results".to_string());

    if !batches.is_empty() {
        let schema = batches[0].schema(); 
        let combined_batch = concat_batches(&schema, &batches)?; 
        for row_index in 0..combined_batch.num_rows() {
            for col in 0..combined_batch.num_columns() {
                let array: &ArrayRef = combined_batch.column(col);
                row.push(array_to_string(array, row_index)?);
            }
        }
    }
    ...
    .create(true)
    .open(output_path)?;
    let mut csv_writer = CsvWriter::from_writer(file); 
    csv_writer.write_record(&row)?; 
    csv_writer.flush()?; 
    Ok(())
}
fn array_to_string(array: &ArrayRef, index: usize) -> Result<String, Box<dyn Error>> {
    use arrow::array::*;
    Ok(match array.data_type() {
        arrow::datatypes::DataType::Utf8 => array.as_any().downcast_ref::<StringArray>().unwrap().value(index).to_string(),
        arrow::datatypes::DataType::Int64 => array.as_any().downcast_ref::<Int64Array>().unwrap().value(index).to_string(),
        arrow::datatypes::DataType::Float64 => array.as_any().downcast_ref::<Float64Array>().unwrap().value(index).to_string(),
